<template>
    <div class="w-100">
        ...
        <div v-show="step===1" class="card">
            <div class="bankname">
                <img draggable="false" class="no-img" v-if="bankName === 'no'" :src="'/img/bank/' + bankName + '.png'"
                    alt="نام بانک">
                <img draggable="false" v-else :src="'/img/bank/' + bankName + '.png'" alt="نام بانک">
                <img draggable="false" src="/icon.png" class="logo">
            </div>
            <div id="get-card">
                <TheMask @input="check_card_number" v-model="card_number" class="cardNumber"
            ...
            <div class="btnc">
                ...
                <span></span>
                <btn @click="changeStep('next')" :disabled="!card_currect"
                    :class="[card_currect ? '' : 'disable-btn', 'py-1 px-2 font-weight-bold rounded']" size="small">
                    مرحله بعدی
                </btn>
            </div>
        </div>
        <div v-show="step === 2" class="card">
            ...
            <div id="get-shaba">
                <TheMask v-model="shaba_number" v-on:input="check_shaba_number" class="cardNumber"
                    :mask="['IR-####-####-####-####-####-####']" placeholder="شماره شبا حساب خود را وارد کنید" />
            </div>
            <div class="btnc">
                ...
                <btn @click="submitBankAcc()" :disabled="!shaba_currect"
                    :class="[shaba_currect ? '' : 'disable-btn', 'py-1 px-2 font-weight-bold rounded']" size="small">
                    مرحله بعدی
                </btn>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        async submitBankAcc() {
            this.loading = true
            let res = await this.$store.dispatch('address/addBankAddress', {name: this.bankName ,card: this.card_number , sheba: this.shaba_number})
            if (JSON.parse(res.ok) === true) {
                this.$fire({
                    title: "عملیات موفق",
                    text: "اطلاعات بانکی شما با موفقیت افزوده گردید...",
                    type: "success",
                    timer: 10000
                });
            } else {
